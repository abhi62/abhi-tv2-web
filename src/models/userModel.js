...
userSchema.pre("save", async function (next) {
  ...
  if (this.isModified("password")) {
    if (passwordValidator.test(this.password)) next();
      ...
      const err = new Error(
        "Password must contain One Uppercase, One lowercase and One number character"
      );
      next(err);
    }
  }
    ...
    if (doc.role === "doctor") {
      ...
      for (let i = 0; i < doc.slots.length; i++) {
        for (let j = 0; j < doc.slots[i].days.length; j++) {
          subslotsArray[days.indexOf(doc.slots[i].days[j])] =
            doc.slots[i].subSlots;
        }
      }
      console.log("subslots op test-1");
      doc.doctor.subslots = subslotsArray;
      console.log("subslots op test-2");
    }
  }
  next();
});
